...
exports.detect = function detect(config, finished) {
		...
		if (!installed) {
			results.issues.push({
				...
				message: __('The Command Line Tools are not installed') + '\n'
					+ __('Titanium requires that the Command Line Tools be installed to develop __native Android modules__.') + '\n'
	...
	async.series(tasks, function (err, bins) {
		...
		async.series(tasks, function (err) {
			...
			run(bins['xcode-select'], '--print-path', function (code, out, _err) {
				if (code) {
