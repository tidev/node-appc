...
exports.detect = function detect(finished, opts) {
	...
	async.parallel([
		function (callback) {
			...
			exec('xcode-select -print-path', function (err, stdout, _stderr) {
				...
				function getSDKs() {
					...
					fs.existsSync(dir) && fs.readdirSync(dir).forEach(function (d) {
						...
						if (fs.existsSync(file) && fs.statSync(file).isDirectory()) {
							...
							if (m && (!opts.minSDK || version.gte(m[2], opts.minSDK))) {
								var ver = m[2];
				...
				if (fs.existsSync(path.join(dir, file)) && /.+\.mobileprovision$/.test(file)) {
					...
					if (i !== -1 && j !== -1) {
						...
						if (!p.ProvisionedDevices || !p.ProvisionedDevices.length) {
							dest = 'distribution';
						} else if (Buffer.from(p.DeveloperCertificates[0].value, 'base64').toString().indexOf('Distribution:') !== -1) {
							dest = 'adhoc';
						}
		...
		function (callback) {
			let result = [];
			exec('security list-keychains', function (err, stdout, _stderr) {
				if (!err) {
					result = result.concat(stdout.split('\n').filter(function (line) { // eslint-disable-line array-callback-return
						var m = line.match(/[^"]*"([^"]*)"/);
