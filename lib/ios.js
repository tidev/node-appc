...
/**
 * Detects the iOS developmeent environment.
	...
	async.parallel([
		function (callback) {
			...
			fs.lstatSync('/Volumes').isDirectory() && fs.readdirSync('/Volumes').forEach(function (dir) {
				...
				fs.existsSync(vol + '/Applications') && fs.lstatSync(vol + '/Applications').isDirectory() && fs.readdirSync(vol + '/Applications').forEach(function (dir) {
