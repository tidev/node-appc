...
async function detectPlatformSpecificNativeModuleInNPMPackage(directory, platformName, platformCount, json, logger) {
	...
	manifest.name = platformMetadata.name || json.titanium.name || manifest.name || json.name;
	manifest.minsdk = platformMetadata.minsdk || json.titanium.minsdk || manifest.minsdk;
	manifest.apiversion = platformMetadata.apiversion || json.titanium.apiversion || parseInt(manifest.apiversion);
	manifest.guid = platformMetadata.guid || json.titanium.guid || manifest.guid;
	manifest.moduleid = platformMetadata.moduleid || json.titanium.moduleid || manifest.moduleid || json.name;
	manifest.architectures = platformMetadata.architectures || json.titanium.architectures || manifest.architectures.split(' ');
	manifest.version = platformMetadata.version || json.titanium.version || manifest.version || json.version;
