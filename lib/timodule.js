...
async function detectNativeModuleViaNPMPackage(singlePackageDir) {
	...
	try {
		...
		if (json && json.titanium && json.titanium.moduleid && json.titanium.type === 'native-module') {
			...
			const platformObject = json.titanium.platform;
			// we need to construct a "module" instance for each platform
			const platformNames = Object.keys(platformObject);
...
async function detectPlatformSpecificNativeModuleInNPMPackage(directory, platformName, platformCount, json) {
	...
	return {
		...
		manifest: {
			...
			minsdk: json.titanium.platform[platformName].minsdk,
			apiversion: json.titanium.platform[platformName].apiversion,
			guid: json.titanium.guid,
			moduleid: json.titanium.moduleid,
			architectures: json.titanium.platform[platformName].architectures
		}
	};
}
