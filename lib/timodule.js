						...
						if (scope[platform] && scope[platform][module.id]) {
							...
							if (info) {
								...
								if (sdkVersion && info.manifest && info.manifest.minsdk && version.gt(info.manifest.minsdk, sdkVersion)) {
									...
									for (var k = 0; !alreadyAdded && k < result.found.length; k++) {
										if (result.found[k].id == tmp.id && result.found[k].version == ver) {
											alreadyAdded = true;
										}
									}
									...
									if (!alreadyAdded) {
										...
										logger && logger.info(__('Found Titanium module id=%s version=%s platform=%s deploy-type=%s path=%s', tmp.id.cyan, originalVersion.cyan, tmp.platform.join(',').cyan, tmp.deployType.join(',').cyan, tmp.modulePath.cyan));
