...
async function detectPlatformSpecificNativeModuleInNPMPackage(directory, platformName, platformCount, json, logger) {
	...
	manifest.name = platformMetadata.name || manifest.name || json.titanium.name || json.name;
	manifest.minsdk = platformMetadata.minsdk || manifest.minsdk || json.titanium.minsdk;
	manifest.apiversion = platformMetadata.apiversion || (manifest.apiversion && parseInt(manifest.apiversion)) || json.titanium.apiversion;
	manifest.guid = platformMetadata.guid || manifest.guid || json.titanium.guid;
	manifest.moduleid = platformMetadata.moduleid || manifest.moduleid || json.titanium.moduleid || json.name;
	manifest.architectures = platformMetadata.architectures || (manifest.architectures && manifest.architectures.split(' ')) || json.titanium.architectures;
	manifest.version = platformMetadata.version || manifest.version || json.titanium.version || json.version;
