		...
		return function(cb) {
			...
			async.parallel(tasks, function () {
				...
				fs.readdirSync(moduleRoot).forEach(function (platform) {
					...
					if (fs.statSync(modulesPath).isDirectory() && !osNamesRegExp.test(platform) && !ignoreDirs.test(platform)) {
						...
						fs.readdirSync(modulesPath).forEach(function (moduleName) {
							...
							if (fs.statSync(modulePath).isDirectory() && !ignoreDirs.test(moduleName)) {
								...
								fs.readdirSync(modulePath).forEach(function (ver) {
									...
									if (fs.statSync(versionPath).isDirectory() && fs.existsSync(manifestFile)) {
