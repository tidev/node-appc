 ...
 * @param {Function} callback - A function to call when done
 */
exports.detect = function detect(searchPaths, config, logger, callback, bypassCache) {
	...
	async.parallel({
		...
		user: function (next) {
			if (config.paths && Array.isArray(config.paths.modules)) {
				detectModules(config.paths.modules, config, logger, next);
			} else {
				next();
			}
		},
 ...
 * @param {Function} callback - A function to call when done
 */
exports.find = function find(modules, platforms, deployType, sdkVersion, searchPaths, config, logger, callback) {
	var result = {
			found: [],
			missing: [],
			incompatible: [],
								...
								if (sdkVersion && info.manifest && info.manifest.minsdk && version.gt(info.manifest.minsdk, sdkVersion)) {
									...
									for (var k = 0; k < result.found.length; k++) {
										if (result.found[k].id == tmp.id) {
											...
											} else if (result.found[k].version == ver) {
												alreadyAdded = true;
											}
										}
									}
