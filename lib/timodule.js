...
async function detectNativeModuleViaNPMPackage(singlePackageDir) {
	...
	if (json && json.titanium && json.titanium.type === 'native-module') {
		...
		return await Promise.all(platformNames.map(p => detectPlatformSpecificNativeModuleInNPMPackage(singlePackageDir, p, platformCount, json)));
	}
	return [];
}
 ...
 */
async function detectPlatformSpecificNativeModuleInNPMPackage(directory, platformName, platformCount, json) {
	...
	const platformMetadadata = (json.titanium.platform && json.titanium.platform[platformName]) || {};
	manifest.name = json.titanium.name || manifest.name;
	manifest.minsdk = platformMetadadata.minsdk || manifest.minsdk;
	manifest.apiversion = platformMetadadata.apiversion || parseInt(manifest.apiversion);
	manifest.guid = json.titanium.guid || manifest.guid;
	manifest.moduleid = json.titanium.moduleid || manifest.moduleid;
	manifest.architectures = platformMetadadata.architectures || manifest.architectures.split(' ');
