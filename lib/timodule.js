...
exports.scopedDetect = function scopedDetect(searchPaths, logger, callback) {
	...
	Object.keys(searchPaths).forEach(function (scope) {
		tasks[scope] = function (next) {
			detectModules(searchPaths[scope], logger, next);
		};
	});
 ...
 * @param {Function} callback - A function to call when done
 */
exports.detect = function detect(searchPaths, logger, callback) {
	if (modules) {
						...
						if (scope[platform] && scope[platform][module.id]) {
							...
							if (info) {
								...
								if (sdkVersion && info.manifest && info.manifest.minsdk && version.gt(info.manifest.minsdk, sdkVersion)) {
									...
									logger && logger.info(__('Found Titanium module id=%s version=%s platform=%s deploy-type=%s', tmp.id.cyan, originalVersion.cyan, tmp.platform.join(',').cyan, tmp.deployType.join(',').cyan));
		...
		return function(cb) {
			...
			async.parallel(tasks, function () {
				...
				fs.readdirSync(moduleRoot).forEach(function (platform) {
					...
					if (fs.lstatSync(modulesPath).isDirectory() && !osNamesRegExp.test(platform) && !ignoreDirs.test(platform)) {
						...
						fs.readdirSync(modulesPath).forEach(function (moduleName) {
							...
							if (fs.lstatSync(modulePath).isDirectory() && !ignoreDirs.test(moduleName)) {
								...
								fs.readdirSync(modulePath).forEach(function (ver) {
									...
									if (fs.lstatSync(versionPath).isDirectory() && fs.existsSync(manifestFile)) {
