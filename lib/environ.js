...
module.exports.getSDK = function (version) {
	version == 'latest' && (version = Object.keys(env.sdks).sort().pop());
...
os.sdkPaths.forEach(function (titaniumPath) {
	...
	if (afs.exists(titaniumPath)) {
		...
		/* Commented out until modules gets further fleshed out...
...
module.exports.getOSInfo = function (callback) {
	...
	osInfo = {
		os: '',
		ostype: process.platform.replace(/darwin/, 'osx'),
		osver: '',
		osarch: (/64/.test(process.arch) ? 64 : 32) + 'bit',
