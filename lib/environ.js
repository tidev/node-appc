	...
	async.series([
		function (next) {
			switch (process.platform) {
				case 'darwin':
					exec('sw_vers', function (err, stdout, _stderr) {
						if (!err) {
				...
				case 'linux':
					if (fs.existsSync('/etc/lsb-release')) {
						const s = fs.readFileSync('/etc/lsb-release').toString(),
						...
						const s = fs.readFileSync('/etc/system-release').toString().split(' ');
				...
				case 'win32':
					exec('wmic os get Caption,Version', function (err, stdout, _stderr) {
						if (!err) {
							var s = stdout.split('\n')[1].split(/ {2,}/);
		...
		exec('npm --version', function (err, stdout, _stderr) {
			if (!err) {
