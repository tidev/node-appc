...
var subprocess = require('./subprocess').subprocess,
	fs = require('fs'),
	path = require('path'),
	cache;
...
exports.detect = function detect(config, opts, finished) {
	...
	switch (process.platform) {
		case 'darwin':
			var kextPath = (config && config.get && config.get('haxm.kext')) || '/System/Library/Extensions/intelhaxm.kext',
				configure = path.join(kextPath, 'Contents/Resources/haxm_configure');
			result.installed = fs.existsSync(kextPath);
			if (result.installed && fs.existsSync(configure)) {
				subprocess(configure, function (err, stdout, stderr) {
					if (!err) {
						result.memlimit = ~~stdout.split('\n').shift();
					}
					finished(result);
				});
			} else {
		...
		case 'win32':
			...
			if (result.installed) {
				subprocess('reg', ['query', 'HKLM\\Software\\haxm\\haxm', '/v', 'MemLimit'], function (err, stdout, stderr) {
					if (!err) {
