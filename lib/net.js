	...
	var ifaces = require('os').networkInterfaces(),
		...
		gateway = {
			darwin: function (ifaces, output, done) {
				exec('netstat -nr', function (err, stdout, _stderr) {
					var m,
						re = /^default\s+([^\s]+).*?(\S+)$/gm;
			...
			linux: function (ifaces, output, done) {
				exec('ip route', function (err, stdout, _stderr) {
					var m,
						re = /^default via ([^\s]+) dev (\S+)\s*$/gm;
					while (m = re.exec(stdout)) {
						if (m[2] && ifaces[m[2]]) {
							ifaces[m[2]].gateway = m[1];
						}
		...
		if (cmd) {
			exec(cmd, function (err, stdout, _stderr) {
				...
				stdout.replace(/\r\n|\r/g, '\n')					// remove all \r
					...
					.replace(/ {3}/g, '~')							// if indented with spaces, mark with ~ so we can match
