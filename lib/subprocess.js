 ...
 * @param {Function} finished - A function to call when done searching
 */
exports.findExecutable = function findExecutable(files, finished) {
	...
	var find = process.platform === 'win32'
			...
			:	function (file, callback) {
				// pray we find 'which'
				exports.run(fs.existsSync('/usr/bin/which') ? '/usr/bin/which' : 'which', file, function (err, stdout, _stderr) {
					if (err) {
						callback(); // go to next item in the queue
					} else {
	...
	queue.push(files.filter(function (p) {
		return !!p;
	}));
};
...
exports.getRealName = function getRealName(file, callback) {
	...
	exec('cmd /c for %A in ("' + file + '") do @echo %~sA', function (code, out, _err) {
