...
exports.lpad = function lpad(s, len, ch) {
	var pre = '',
		ns = String(s).replace(/\u001b\[\d+m/g, ''), // eslint-disable-line no-control-regex
		i = ns.length;
	ch = ch || ' ';
	while (i++ < len) {
		pre += ch;
...
exports.rpad = function rpad(s, len, ch) {
	var ns = String(s).replace(/\u001b\[\d+m/g, ''), // eslint-disable-line no-control-regex
		i = ns.length;
	ch = ch || ' ';
	while (i++ < len) {
		ns += ch;
...
exports.renderColumns = function renderColumns(items, margin, maxwidth) {
	const longest = items.reduce(function (a, b) {
		return Math.max(a, b.stripColors.length);
	}, 0) + 6;
	const width = maxwidth ? Math.min(maxwidth, process.stdout.columns || 80) : (process.stdout.columns || 80);
	...
	for (let i = 0; i < rows; i++) {
		...
		for (let j = 0; j < len; j += rows) {
			if (j + i < len) {
				buffer += items[i + j];
				const spaces = longest - items[i + j].stripColors.length;
