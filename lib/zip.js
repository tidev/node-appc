...
exports.zip = function (target, result, cwd, callback) {
	var targets = [],
		child,
		stdout = '',
		stderr = '';
	...
	if (target) {
		...
		targets.forEach(function (item, index) {		
			targets[index] = afs.resolvePath(item);
		});
	}

	if (process.platform === 'win32') {
		if (targets.length == 0) {
			//zip all files in current directory if target files list is empty
			child = spawn(path.resolve(module.filename, '..', '..', 'tools','7zip', '7za.exe'), ['a', result, cwd + '/*', '-tzip']);
		} else {
			//7za gets list of targets from stdio if switch -si present
			child = spawn(
