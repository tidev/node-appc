...
exports.detect = function (finished, sdkPath, ndkPath) {
	if (cached) return finished(cached);
	
	var result = {
			sdkPath: sdkPath || process.env.ANDROID_SDK || '',
			ndk: {
				path: ndkPath || process.env.ANDROID_NDK || '',
				version: ''
			},
			targets: {},
			avds: []
	...
	exe = result.exe = path.join(sdkPath, 'tools', process.platform == 'win32' ? 'android.bat' : 'android');
	if (exe.match(' ')) {
	...
	async.parallel([
		...
		function (next) {
			...
			exec(exe + ' list targets', function (error, stdout, stderr) {
				...
				if (!error && !stderr) {
					...
					// Note: for some reason getting the list of targets doesn't always work on Windows the first time
					console.error('Could not get the list of Android Targets: ' + stderr);
		...
		function (next) {
			exec(exe + ' list avd', function (error, stdout, stderr) {
				...
				if (!error && !stderr) {
					...
					// Note: for some reason getting the list of AVDs doesn't always work on Windows the first time
					console.error('Could not get the list of Android AVDs: ' + stderr);
					errorEncountered = true;
				}
				next();
			});
		...
		errorEncountered ? finished() : finished(cached = result);
	});
};
