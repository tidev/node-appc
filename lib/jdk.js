...
exports.detect = function detect(config, opts, finished) {
	...
	if (cache && !opts.bypassCache) return finished && finished(cache);
	...
	if (process.platform == 'darwin') {
		...
		subprocess('/usr/libexec/java_home', function (err, stdout, stderr) {
			if (err) {
				result.issues.push({
	...
	function checkCommands() {
		...
		async.parallel(tasks, function (err, executables) {
			...
			if (executables.javac) {
				subprocess(executables.javac, '-version', function (err, stdout, stderr) {
