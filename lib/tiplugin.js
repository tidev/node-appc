 ...
 */
exports.scopedDetect = function scopedDetect(searchPaths, logger, callback) {
	...
	Object.keys(searchPaths).forEach(function (scope) {
		tasks[scope] = function (next) {
			detectPlugins(searchPaths[scope], logger, next);
		};
	});
...
/**
 * Scans search paths for Titanium CLI plugins. This function will scan all
 * known Titanium SDK locations.
 * @param {Array<String>} searchPaths - An array of paths to search for Titanium CLI plugins
 * @param {Object} logger - A logger instance
 * @param {Function} callback - A function to call when done
 */
 ...
 * specified modules are found or missing.
 * @param {Array<Object>} modules - An array of plugins to search for
 * @param {Array<String>|String} platforms - An array of platform names (if the platform has more than one name)
 * @param {String} deployType - The deploy type to filter by
...
function detectPlugins(searchPaths, logger, callback) {
	var results = {},
		ignoreDirs = /^(\.svn|\.git|\.hg|\.?[Cc][Vv][Ss]|\.bzr)$/;
	...
	searchPaths.forEach(function (pluginRoot) {
		...
		var packageFile = path.join(pluginRoot, 'package.json'),
			packageFileExists = fs.existsSync(packageFile) && (fs.existsSync(path.join(pluginRoot, 'commands')) || fs.existsSync(path.join(pluginRoot, 'hooks'))),
		...
		if (packageFileExists || pluginFileExists) {
			...
			fs.readdirSync(pluginRoot).forEach(function (pluginName) {
				...
				if (fs.lstatSync(pluginsPath).isDirectory() && !ignoreDirs.test(pluginName)) {
					// we have a plugin directory

					function processDir(versionPath, dest) {
						var packageFile = path.join(versionPath, 'package.json'),
							pluginFile = path.join(versionPath, 'plugin.py'),
							commandsDir = path.join(versionPath, 'commands'),
							hooksDir = path.join(versionPath, 'hooks'),
							pluginFileExists = fs.existsSync(pluginFile),
							plugin,
							jsfile = /\.js$/,
							ignore = /^[\._]/;

						if (pluginFileExists || fs.existsSync(commandsDir) || fs.existsSync(hooksDir)) {
								...
								fs.readdirSync(hooksDir).forEach(function (filename) {
									...
									if (fs.statSync(file).isFile() && jsfile.test(filename) && !ignore.test(filename)) {
										...
										try {
											var mod = { exports: {} },
												exp;
											(new Function('require', 'module', 'exports', '__dirname', '__filename', fs.readFileSync(file).toString()))(require, mod, mod.exports, hooksDir, filename);
											exp = mod.exports || {};
											exp.name && (info.name = exp.name);
											exp.cliVersion && (info.cliVersion = exp.cliVersion);
											exp.version && (info.version = exp.version);
										} catch (ex) {}
							...
							if (pluginFileExists) {
								dest.legacyPluginFile = pluginFile;
							}
					...
					if (fs.existsSync(path.join(pluginsPath, 'plugin.py')) || fs.existsSync(path.join(pluginsPath, 'commands')) || fs.existsSync(path.join(pluginsPath, 'hooks'))) {
						// we have a plugin without a version folder or a project level plugin
						results[pluginName] || (results[pluginName] = {});
						processDir(pluginsPath, results[pluginName].unknown = {});
					} else {
						// loop through versions
						fs.readdirSync(pluginsPath).forEach(function (ver) {
							var dir = path.join(pluginsPath, ver);
							if (fs.statSync(dir).isDirectory() && !ignoreDirs.test(dir)) {
								results[pluginName] || (results[pluginName] = {});
								processDir(path.join(pluginsPath, ver), results[pluginName][ver] = {});
							}
						});
					}
				}
