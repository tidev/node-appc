...
function detectPlugins(searchPaths, config, logger, callback) {
	const results = {},
		ignoreDirs = new RegExp(config && config.get('cli.ignoreDirs') || '^(.svn|.git|.hg|.?[Cc][Vv][Ss]|.bzr)$'); // eslint-disable-line security/detect-non-literal-regexp
	...
	searchPaths.forEach(function (pluginRoot) {
		...
		if (packageFileExists || pluginFileExists) {
			...
			const plugin = results[pluginName] || (results[pluginName] = {});
			plugin['-'] = {
							...
							fs.readdirSync(hooksDir).forEach(function (filename) {
								...
								if (fs.statSync(file).isFile() && jsfile.test(filename) && !ignore.test(filename)) {
									...
									try {
										...
										const mod = require(file); // eslint-disable-line security/detect-non-literal-require
