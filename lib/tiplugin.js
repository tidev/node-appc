			...
			fs.readdirSync(pluginRoot).forEach(function (pluginName) {
				...
				if (fs.lstatSync(pluginsPath).isDirectory() && !ignoreDirs.test(pluginName)) {
					...
					function processDir(versionPath, dest) {
						...
						if (pluginFileExists || afs.exists(commandsDir) || afs.exists(hooksDir)) {
							...
							if (afs.exists(commandsDir) && fs.statSync(commandsDir).isDirectory()) {
							...
							if (afs.exists(hooksDir) && fs.statSync(hooksDir).isDirectory()) {
					...
					var packageFileExists = afs.exists(path.join(pluginsPath, 'package.json')),
						pluginName = path.basename(pluginsPath);

					if (afs.exists(path.join(pluginsPath, 'plugin.py')) || afs.exists(path.join(pluginsPath, 'commands')) || afs.exists(path.join(pluginsPath, 'hooks'))) {
