 ...
 * @param {Function} [callback] - A function to call after the analytics have been saved
...
exports.send = function send(args, callback) {
	...
	async.parallel({
		...
		var eventsDir = path.join(titaniumHomeDir, 'events'),
			...
			mid = results.mid,
			sessionCookie = args.cookie && args.cookie.match(/(PHPSESSID=[A-Za-z0-9]+)/),
			cookie = (sessionCookie ? sessionCookie[1] + ';' : '') + 'uid=' + args.guid; // no, this is not a bug... it really is called uid and expects the guid
		...
		function writeEvent(type, event, id, ts, data) {
			var file = path.join(eventsDir, (new Date).toISOString() + '_' + (Math.floor(Math.random() * 9000) + 1000) + '.json'),
				json = mix({
					event: event,
					type: type,
					sid: sid,
 ...
 * cloud servers.
 * @param {Object} args - Send arguments
 * @param {String} args.cookie - The session cookie
 * @param {Boolean} [args.debug=false] - Displays debug logging when true
...
process.on('message', function (args) {
	if (!args || !args.titaniumHomeDir || !args.url || !args.cookie) return;
